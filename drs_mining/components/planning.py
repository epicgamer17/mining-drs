...
class MiningPriority(Enum):
    '''Monthly mine-level priority selected by the existing tactical controller.'''
...
class StrategicYearTarget:
    ...
    Defaults are zero so the current simulation behaviour remains unchanged until
    explicit strategic targets are supplied in the configuration.
    '''
