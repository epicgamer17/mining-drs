...
optimal truck dispatch weights between High-Ore1 Face 1 (85% Ore 1) and Balanced Face 2 (55% Ore 1):
...
# ---------------------------------------------------------------------------
# Two-Area Analytical Blending Simulation Module
# ---------------------------------------------------------------------------
class TwoAreaAnalyticalBlendingSimulation(drs.Module):
    """Underground DES simulation featuring analytical operational face-allocation blending (Slide 29)."""
    ...
    def __init__(
        ...
        # 8. Analytical Face Allocation Variables
    ...
    # -- Analytical Face Allocation (Slide 29) --------------------------------
    def _compute_analytical_face_allocation(self, plant_draw: PlantDrawRates):
        """Solves exact face allocation rates and weights based on active parcels and mode draw."""
    ...
    def step(self, dt: float):
        ...
        if self.is_area2_locked():
            ...
            w1 = float(self.analytical_face1_weight.value)
            w2 = float(self.analytical_face2_weight.value)
            f_blend = (
                self.face1.active_parcel_ore_fraction.value * w1
    ...
    # -- Dispatch Policy (Driven by Analytical Weights w1, w2) ----------------
    def _try_dispatch(self, tr: Truck) -> bool:
        ...
        target_face_id = self._select_face_by_analytical_weights()
    ...
    def _select_face_by_analytical_weights(self) -> int:
        """Selects face based on Slide 29 closed-form analytical weights w1, w2."""
        if self.is_area2_locked():
            return 1
        ...
        self.history_records.append(
            {
                ...
                "analytical_face1_weight": w1,
                "analytical_face2_weight": w2,
                "analytical_face1_rate_target": self.analytical_face1_rate_target.value,
                "analytical_face2_rate_target": self.analytical_face2_rate_target.value,
                "analytical_blend_feasible": self.analytical_blend_feasible.value,
...
# Visual Dashboard
# ---------------------------------------------------------------------------
def plot_analytical_blending_dashboard(
    df: pd.DataFrame,
    ...
    plot_ore_with_modes(
        ...
        palette=palette,
        hlines=[
            {"y": 60000.0, "color": "black", "linestyle": "--", "linewidth": 1.5, "label": "Target Total (60k)"},
            {"y": 20400.0, "color": "red", "linestyle": ":", "linewidth": 2.0, "label": "Critical Ore 2 (20.4k)"},
        ],
        ax=dash[1],
    )
    ...
    # 2. Analytical Face Allocation Weights (Slide 29)
    plot_time_series(
        df,
        y_columns=["analytical_face1_weight", "analytical_face2_weight"],
        title="Analytical Face Allocation Weights w1 (Face 1) & w2 (Face 2) [Slide 29]",
        is_step=True,
        ax=dash[2],
    )
    if unlock_time is not None:
    ...
    dash[2].set_ylabel("Dispatch Weight (Fraction)")
    dash[2].set_ylim(-0.05, 1.05)
    dash[2].legend(loc="upper right")

    # 3. Whole-Mine Cumulative Operating NPV
    plot_dual_axis_step(
        df,
        ...
        title="Cumulative Operating NPV & Discounted Cash Flow Rates",
        ax=dash[3],
    )
    if unlock_time is not None:
        dash[3].axvline(
    ...
    # 5. Strategic Trajectory Progress Ratios
    plot_time_series(
        df,
        y_columns=[
    ...
    dash[5].axhline(0.90, color="red", linestyle=":", label="Tolerance Threshold (0.90)")
    dash[5].axhline(1.00, color="gray", linestyle="--", label="Target Trajectory (1.00)")
    dash[5].set_ylabel("Trajectory Ratio")
    dash[5].legend(loc="upper right")
    ...
    plot_time_series(
        ...
        title="Underground Haulage Fleet Activity & Queues",
        is_step=True,
        ax=dash[8],
    )
    ...
    plot_mode_distribution(
        ...
        mode_col="active_operating_mode_name",
        time_col="time",
        title="Mode Distribution (% Time Spent)",
        palette=palette,
        ax=dash[9],
    )
    ...
    print(f"Saved analytical blending dashboard to '{output_path}'.")
    return dash
...
def run_two_area_analytical_simulation(
    ...
    """Runs the two-area analytical face-allocation simulation."""
    ...
    eng = drs.DRSEngine(max_step_size=DT_MAX)
    eng.register(sim)
    eng.on_step(sim.on_event)

    if total_days is not None:
        sim.horizon_sec = total_days * 86400.0
        eng.run(until=sim.horizon_sec)
    else:
        sim.total_ore_to_extract = warmup_ore
        sim.face1.total_ore_to_extract = warmup_ore
        sim.face2.total_ore_to_extract = warmup_ore
        eng.run(until=float("inf"))
    ...
    if plot and len(df_prepared) > 0:
        plot_analytical_blending_dashboard(df_prepared)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Two-Area Analytical Face-Allocation Blending Simulation (Appendix A & B)"
    )
