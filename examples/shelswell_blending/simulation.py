...
3. Dynamic Target-Driven Fleet Dispatch:
...
class TruckPhase(Enum):
    IDLE = "idle"  # parked on surface, no operator or awaiting dispatch
    EMPTY = "empty"  # empty travel surface -> underground loadout
    WAIT_LOAD = "wait_load"  # queued at level LHD
    SPOT_LOAD = "spot_load"  # spotting at loadout
    ACQUIRE = "acquire"  # waiting for LHD
    LOADING = "loading"  # active loading
    LOADED = "loaded"  # loaded travel loadout -> surface tip
    WAIT_DUMP = "wait_dump"  # queued at dump tip
    DUMPING = "dumping"  # active dumping into stockpile / dump
...
class Loadout:
    idx: int
    level: int
    bay_type: str  # "ORE_1", "ORE_2", "WASTE"
...
class ShelswellBlendingHaulage(drs.Module):
    """Hybrid simulation combining Shelswell DES with Plant Operating Modes & DRS Stockpiles.

    Supervisory OperatingModeController & MetallurgicalPlant determine campaign modes
    and set daily targets for Ore 1, Ore 2, and Waste. The underground DES fleet is
    dispatched dynamically to meet these targets.
    """
    ...
    def on_event(self, t: float):
        ...
        # Update Operating Mode & Daily Targets from Plant
    ...
    def _update_operating_mode_and_targets(self):
        """Updates supervisory mode and sets current daily haulage targets."""
    ...
    def _finish_loading(self, tr: Truck):
        lo = self.loadouts[tr.target_loadout]
        cap = ORE_PAYLOAD if "ORE" in tr.payload_type else WASTE_PAYLOAD
    ...
    def _start_dump(self, site: DumpSite, tr: Truck):
        ...
        # Continuous DRS flow rate into stockpile
    ...
    def _finish_dumping(self, tr: Truck):
        ...
        # Bookkeeping of daily & cumulative hauled totals
        payload = tr.current_payload
        if tr.payload_type == "ORE_1":
            self.daily_hauled_ore1 += payload
            self.ore1_hauled.value += payload
        elif tr.payload_type == "ORE_2":
            self.daily_hauled_ore2 += payload
            self.ore2_hauled.value += payload
        else:
            self.daily_hauled_waste += payload
            self.waste_hauled.value += payload
    ...
    def _travel_time(self, tr: Truck, loaded: bool) -> float:
        ...
        d_surf = ROM_SURFACE_M if is_ore else WASTE_SURFACE_M
        d_lvl = ORE_LEVEL_DRIFT_M if is_ore else WASTE_LEVEL_DRIFT_M
    ...
    def _record_telemetry(self, plant_draw: PlantDrawRates):
        ...
        n_waiting_load = sum(
            1 for tr in self.trucks if tr.phase == TruckPhase.WAIT_LOAD
        )
        n_waiting_dump = sum(len(site.queue) for site in self.dump_sites.values())
        n_refueling = sum(
        ...
        self.history_records.append(
            {
                ...
                "cumulative_extracted_mass": self.ore1_hauled.value
                + self.ore2_hauled.value,
    ...
    # -- Runner --------------------------------------------------------------
    def run(self, total_days: float = DAYS_IN_YEAR) -> pd.DataFrame:
        """Runs the hybrid simulation and returns telemetry dataframe."""
...
def print_simulation_statistics(sim: ShelswellBlendingHaulage, df: pd.DataFrame):
    """Prints comprehensive operational summary statistics."""
    total_days = sim.gt.value / 86400.0
    total_ore_hauled = sim.ore1_hauled.value + sim.ore2_hauled.value
    total_waste_hauled = sim.waste_hauled.value
    ...
    print(f"Total Ore 1 Hauled:        {sim.ore1_hauled.value:,.1f} t ({sim.ore1_hauled.value / total_days:.1f} t/d)")
    print(f"Total Ore 2 Hauled:        {sim.ore2_hauled.value:,.1f} t ({sim.ore2_hauled.value / total_days:.1f} t/d)")
    print(f"Total Waste Hauled:        {total_waste_hauled:,.1f} t ({total_waste_hauled / total_days:.1f} t/d)")
...
def plot_shelswell_blending_dashboard(
    ...
    """Builds and saves the multi-panel diagnostics dashboard matching blending_modes style."""
    ...
    dash = Dashboard(
        ...
        figsize=figsize,
        sharex=False,
        title="Shelswell DES + DRS Blending Modes Comprehensive Diagnostics",
    )
    dash.link_xaxes([0, 1, 2, 3, 4, 5, 8])
    ...
    # 2. Daily Targets vs Hauled Production
    plot_time_series(
        df,
        y_columns=[
            "daily_target_ore1",
        ...
        title="Daily Extraction Targets vs Underground Hauled Production (t/d)",
        is_step=True,
        ax=dash[2],
    )

    # 3. Safety Margin: Ore 1
    plot_safety_margin(
        df,
    ...
    # 5. Underground Haulage Fleet Activity & Queuing
    plot_time_series(
        df,
        y_columns=[
            "active_trucks",
    ...
    # 6. Mode Distribution (% of Time Spent)
    plot_mode_distribution(
        df,
        mode_col="active_operating_mode_name",
        time_col="time",
    ...
    # 8. Cumulative Production Deficit by Mode
    plot_attributed_deficit(
        df,
        time_col="time",
        mode_col="active_operating_mode_name",
    ...
    # 9. Mode Efficiency (Time Spent vs Deficit Caused)
    plot_deficit_disparity(
        df,
        mode_col="active_operating_mode_name",
...
def run_shelswell_blending_simulation(
    ...
    """Builds and runs the hybrid simulation."""
    ...
    df = sim.run(total_days=total_days)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=float,
        default=365.0,
        help="Total simulation duration in days (default: 365.0)",
    )
    parser.add_argument(
        "--trucks",
        type=int,
