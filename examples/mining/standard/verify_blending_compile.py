...
def main():
    ...
    print(f"Reference run completed in {engine_ref.step_count} steps. End time: {engine_ref.current_time}")
    ...
    print(f"Compiled run completed in {engine_compiled.step_count} steps. End time: {engine_compiled.current_time}")
    ...
    for path, ref_val in ref_values.items():
        ...
        if isinstance(ref_val, (int, float)) and isinstance(comp_val, (int, float)):
            if abs(ref_val - comp_val) > 1e-9:
                print(f"Mismatch on '{path}': Reference={ref_val}, Compiled={comp_val} (diff={abs(ref_val-comp_val):.2e})")
                mismatches += 1
        else:
            if ref_val != comp_val:
    ...
    if mismatches == 0:
        print("SUCCESS: Dynamic compilation produced 100% mathematically identical behavior!")
    else:
        print("FAILURE: Mismatches found between reference and compiled simulation runs.")
        sys.exit(1)
    ...
    print(f"Re-compiled run completed in {engine_recompiled.step_count} steps. End time: {engine_recompiled.current_time}")
    ...
    for path, ref_val in ref_values.items():
        ...
        if recomp_val is None:
            ...
            if abs(ref_val - recomp_val) > 1e-9:
                print(f"Mismatch in recompiled run on '{path}': Reference={ref_val}, Recompiled={recomp_val} (diff={abs(ref_val-recomp_val):.2e})")
                mismatches_recomp += 1
        else:
            if ref_val != recomp_val:
                print(f"Mismatch in recompiled run on '{path}': Reference={ref_val}, Recompiled={recomp_val}")
                mismatches_recomp += 1
            
    print(f"Recompiled mismatches: {mismatches_recomp}")
    if mismatches_recomp == 0:
        print("SUCCESS: Bidirectional round-trip (compile -> serialize -> compile) is 100% correct!")
    else:
