...
def plot_theoretical_fleet_diminishing_returns():
    ...
    for truck_alloc in truck_allocs:
        ...
        # 2. Cycle Time
        truck_cycle_time = travel_time + truck_loading_time_hours + c.truck_dump_time_hours + traffic_delay
        ...
        if mf < 1.0:
            ...
            rate = ((lhd_alloc / c.loader_cycle_time_hours) * c.loader_payload_tonnes * 24.0)
    ...
    axes[0].set_title(f'Diminishing Returns: Throughput vs Fleet Size ({int(lhd_alloc)} LHDs Constant)', fontsize=14)
    ...
    axes[1].set_title('Fleet Match Factor (MF > 1 means Loaders are Bottleneck)', fontsize=14)
    ...
    axes[2].set_title('Traffic Effect: Cycle Time Increases linearly with Trucks', fontsize=14)
    axes[2].grid(True, alpha=0.3)
    
    plt.tight_layout()
    fig.savefig('Diminishing_Returns_Theoretical.png', dpi=300)
    print("Saved Diminishing_Returns_Theoretical.png")
    plt.close(fig)
...
def generate_infinite_vs_constrained_plots():
    ...
    cases = [
        ("Severely Constrained (10 Trucks, 40kt/d Target)", replace(
            base_config, 
        ...
        ("Heavily Constrained (20 Trucks, 40kt/d Target)", replace(
            base_config, 
        ...
        ("Maximum Capacity (40 Trucks, 40kt/d Target)", replace(
            base_config, 
        ...
        ("Infinite Fleet (200 Trucks, 200 LHDs, 40kt/d Target)", replace(
            base_config, 
    ...
    for label, config in cases:
        ...
        df, summary = _run_capacity_case(label, config, max_time=60.0, equal_allocation=False)
        frames.append(df)
    ...
    for (label, group), color in zip(combined.groupby("scenario", sort=False), colors):
        ...
        axes[0].plot(group["time"], group["total_achieved_extraction_rate"], label=label, color=color, alpha=0.8)
        
        # 2. Capacity Gap Rate
        axes[1].plot(group["time"], group["capacity_gap_rate"], label=label, color=color, alpha=0.8)
        
        # 3. Ore 2 Stockpile
        axes[2].plot(group["time"], group["Ore2Stock_mass"], label=label, color=color, alpha=0.8)
        
    # Plot target required rate on axis 0
    axes[0].plot(frames[-1]["time"], frames[-1]["total_target_extraction_rate"], 'k--', label="Target Required Rate", alpha=0.6)
    
    axes[0].set_title("Simulation: Achieved Extraction Rate under varying Fleet Constraints", fontsize=14)
    axes[0].set_ylabel("Extraction Rate (t/d)")
    axes[0].grid(True, alpha=0.3)
    axes[0].legend()
    
    axes[1].set_title("Simulation: Lost Rate (Deficit) due to Fleet Limits", fontsize=14)
    ...
    axes[2].axhline(y=base_config.critical_ore2_level, color='red', linestyle=':', label="Critical Limit")
    axes[2].grid(True, alpha=0.3)
    axes[2].legend()
    
    plt.tight_layout()
    fig.savefig("Infinite_vs_Constrained_Simulation.png", dpi=300)
    print("Saved Infinite_vs_Constrained_Simulation.png")
    plt.close(fig)
