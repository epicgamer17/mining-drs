...
1. Two Distinct Mining Areas / Faces with Stochastic Geological Facies:
   - Area 1 (Face 1, Level 3/4): Low Ore 2 grade (mean ore fraction: 0.15, std: 0.05).
   - Area 2 (Face 2, Level 6/7): High Ore 2 grade (mean ore fraction: 0.45, std: 0.05).
   - Blended average across both areas equals 0.30 (matching single-face baseline).
2. Shelswell (2017) DES Haulage Engine:
   - AD30 haulage trucks, shared LHD loaders per face, operator pooling, workable seat time.
   - Shift calendar (10.5 h workable / 1.5 h gap), 85% mechanical availability.
   - Passing bay traffic congestion, surface fuel depot, continuous surface dump stations.
   - Buffer-aware dispatch throttling maintaining total stockpile at target buffer (60,000 t).
...
3. Strategic & Tactical Planning Framework:
   ...
   - Monthly Tactical Review (every 30 days): Evaluates trajectory progress ratios.
   ...
   - Fleet Reservation: When in DEVELOPMENT priority, reserves extra haulage capacity
     specifically to advance underground development metres (5.0 m/truck-day).
"""
...
# have been disabled (set to 0) in this two-area planning baseline. In the original
...
# ---------------------------------------------------------------------------
# Two-Area Strategic & Tactical Planning Hybrid Simulation Module
# ---------------------------------------------------------------------------
class TwoAreaStrategicBlendingSimulation(drs.Module):
    """Two-Area Strategic & Tactical Planning DES Blending Modes Simulation.

    - Area 1 (Face 1 at Level 3): mean ore fraction f1 = 0.15 (low Ore 2).
    - Area 2 (Face 2 at Level 6): mean ore fraction f2 = 0.45 (high Ore 2).
    - Annual Strategic Targets & Monthly Tactical Review governing MiningPriority.
    ...
    def step(self, dt: float):
        ...
        # Blended routing fraction across active faces (mean 0.30)
        f_blend = (
        ...
        # Development rate is driven by reserved development trucks
        reserved_trucks = float(self.development_priority_reserved_trucks.value)
        # Also idle trucks during off-peak buffer conditions add to development
        ...
        dev_trucks = max(reserved_trucks, float(available_extra))
        ...
        self.cumulative_mine_development.value += (
            self.development_rate_m_per_day.value * dt_days
        )
    ...
    def _update_strategic_tactical_review(self):
        ...
        if (
            ...
            # Select Mining Priority based on Trajectory Deficits
    ...
    def _try_dispatch(self, tr: Truck) -> bool:
        ...
        # Choose Face (Area 1 vs Area 2) based on Blending Needs
...
# ---------------------------------------------------------------------------
def print_statistics(plant, sim: TwoAreaStrategicBlendingSimulation):
...
def print_strategic_tactical_review_table(df: pd.DataFrame):
    """Prints tabular summary of monthly tactical reviews and trajectory progress."""
    print("\n" + "=" * 90)
    print(" STRATEGIC & TACTICAL PLANNING REVIEW LOG")
    print("=" * 90)
...
def print_simulation_statistics(
    sim: TwoAreaStrategicBlendingSimulation, df: pd.DataFrame
):
    ...
    print("\n" + "=" * 70)
    print(" TWO-AREA STRATEGIC DES + DRS BLENDING MODES RESULTS")
    print("=" * 70)
...
def plot_two_area_strategic_dashboard(
    ...
    # 2. Tactical Mining Priority & Strategic Trajectory Progress Ratios
    plot_time_series(
        df,
        y_columns=[
            "development_trajectory_ratio",
        ...
        title="Strategic Trajectory Progress Ratios (1.0 = On Schedule, <0.90 = Deficit)",
        is_step=True,
        ax=dash[2],
    )
    dash[2].axhline(0.90, color="red", linestyle=":", label="Tolerance Threshold (0.90)")
    dash[2].axhline(1.00, color="gray", linestyle="--", label="Target Trajectory (1.00)")
    dash[2].set_ylabel("Trajectory Ratio")
    dash[2].legend(loc="upper right")

    # 3. Cumulative Mine Development (Metres) & Reserved Trucks
    plot_dual_axis_step(
        df,
        y1_col="cumulative_mine_development",
        y2_col="development_priority_reserved_trucks",
        y1_label="Cumulative Dev (m)",
        y2_label="Reserved Dev Trucks",
        title="Cumulative Underground Development & Tactical Truck Reservations",
        ax=dash[3],
    )
...
def run_two_area_strategic_simulation(
    ...
    seed: int = 42,
    plot: bool = True,
) -> Tuple[TwoAreaStrategicBlendingSimulation, pd.DataFrame]:
    """Builds and runs the Two-Area strategic simulation."""
    ...
    sim = TwoAreaStrategicBlendingSimulation(
    ...
    print_transition_log(
        ...
        target_ore_stock_level=target_ore_stock_level,
        label="Two-Area Strategic Blending",
    )
    print_deficit_by_mode(
        df_prepared,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run Two-Area Strategic & Tactical DES Blending Modes Simulation"
    )
    parser.add_argument(
