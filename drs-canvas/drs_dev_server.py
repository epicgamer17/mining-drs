...
def react_flow_to_drs_flat(nodes, edges):
    ...
    if not parent_exists:
        drs_nodes.append(
            {"id": "", "class": "DRSModel", "layout": {"x": 0, "y": 0}, "variables": {}}
        )
...
class DevServerHandler(http.server.BaseHTTPRequestHandler):
        ...
        if self.path == "/api/topology":
            ...
            if os.path.exists(file_path):
                try:
                    ...
                    self.wfile.write(
                        json.dumps({"error": f"Failed to load file: {e}"}).encode(
                            "utf-8"
                        )
                    )
                    return
            ...
            self._set_headers(200)
            self.wfile.write(
                json.dumps({"nodes": DEFAULT_NODES, "edges": DEFAULT_EDGES}).encode(
                    "utf-8"
                )
            )
        else:
            ...
            self.wfile.write(
                json.dumps({"error": f"Path '{self.path}' not found"}).encode("utf-8")
            )
    ...
    def do_POST(self):
        ...
        try:
            ...
            self.wfile.write(
                json.dumps({"error": f"Invalid JSON body: {e}"}).encode("utf-8")
            )
            return
        ...
        if self.path == "/api/topology":
            ...
            try:
                ...
                self._set_headers(500)
                self.wfile.write(
                    json.dumps({"error": f"Failed to save topology state: {e}"}).encode(
                        "utf-8"
                    )
                )
            ...
            try:
                ...
                self.wfile.write(
                    json.dumps(
                        {"status": "error", "message": str(e), "details": err_details}
                    ).encode("utf-8")
                )
